...
## net/dial.go
> dial.go文件中其实并没有实际发起连接，起作用更多是在连接前进行准备工作，如解析网络类型、从addr解析ip地址，实际发起连接在tcpsock_posix.go、udpsock_posix.go中。
...
DialContext调用：
- dialParallel，负责将v4地址和v6地址分开，然后尝试连接v4地址池，一定延时（dialer.fallbackDelay）后尝试v6地址池。
- dialSerial，尝试连接，直到碰到第一个错误或者第一个连接成功后返回。
...
dialSingle进行对单个Ip地址的分配，其解决了兼容不同网络类型的问题。
