...
- (void)setTabbarItems:(NSArray *)tabbarItems{
    if (_tabbarItems != tabbarItems) {
        ...
        for (DLScrollTabbarItem *item in tabbarItems) {
            ...
            imageView.tag = kRedDotViewTag + i;
            imageView.backgroundColor = _redDotColor ? _redDotColor : [UIColor redColor];
            imageView.layer.cornerRadius = 3.5f;
            imageView.layer.masksToBounds = YES;
            imageView.frame = CGRectMake(CGRectGetMaxX(label.frame), label.frame.origin.y - 4, 7, 7);
...
- (void)reloadTabBarView
{
    for (int i = 0; i < [_tabbarItems count]; i ++) {
        DLScrollTabbarItem * item = [_tabbarItems objectAtIndex:i];
        
        UIView * bkgView = [scrollView_ viewWithTag:kViewTagBase+i];
        UIView * imageView = [bkgView viewWithTag:kRedDotViewTag + i];
        if(imageView) {
            [imageView setHidden:!item.hasNewMsg];
        }
    }
}
